...
module.exports = function (s, conf) {
  ...
  console.log("Conf in ENGINE" + util.inspect(conf.websocketApiInstance, false, null)) 
  let so = s.options
  ...
  function pushMessageToWebSocket(messageType, date) {
    if (so.mode === 'live' || so.mode === 'paper') {
      websocketApi.send(messageType, date)
    }
  }
  ...
  function syncBalance (cb) {
	...
	var data = {
        ...
        pushMessageToWebSocket("balance",data);
  ...
  function executeOrder (trade, type) {
	...
	let res = {
	    trade:my_trade,
	    myTrades: s.my_trades
	}
        pushMessageToWebSocket("buy",res)
      }
	...
	let res = {
        ...
        pushMessageToWebSocket("sell",res)
      }
  ...
  function writeReport (is_progress, blink_off) {
    ...
    //
    webSocketMsg.time = moment(is_progress ? s.period.latest_trade_time : tb(s.period.time).resize(so.period_length).add(1).toMilliseconds()).format('YYYY-MM-DD HH:mm:ss')
    //  
    if (s.lookback[0]) {
	...
	//
	webSocketMsg.diff = z(8, formatPercent(diff), ' ')
    }
    else {
    ...
    //
    webSocketMsg.volume = volume_display
    //
	...
	//
	//webSocketMsg.type = formatPercent(s.last_trade_worth)
    ...
    if (orig_capital) {
      ...
      //
      webSocketMsg.asset = n(s.balance.asset).format(s.asset === 'BTC' ? '0.00000' : '0.00000000')
      ...
      //
      webSocketMsg.currency = n(s.balance.currency).format(isFiat() ? '0.00' : '0.00000000')
      ...
      //
      webSocketMsg.profit = formatPercent(profit)
      ...
      //
      webSocketMsg.over_buy_hold_pct = formatPercent(over_buy_hold_pct)
      pushMessageToWebSocket('writeReport',webSocketMsg)
    }
    if (!is_progress) {
  ...
  function onTrades(trades, is_preroll, cb) {
	...
	my_trades: s.my_trades,
	trades: trades,
	asset: s.asset,
	currency: s.currency,
	exchange: s.exchange
    }
    pushMessageToWebSocket("chart",mes)
  ...
  return {
    ...
    pushMessageToWebSocket: pushMessageToWebSocket
  }
}
